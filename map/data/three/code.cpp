...
bool check2() {  // Q.insert
    ...
    for (int i = 1; i <= 100; i++)
        swap(num[rand() % 50 + 1], num[rand() % 50 + 1]);
...
bool check3() {  // find remove
    ...
    for (int i = 1; i <= 60000; i++)
        swap(num[rand() % 30000 + 1], num[rand() % 30000 + 1]);
...
bool check5() {  // insert && remove
    ...
    while (!stdQ.empty()) {
        if (Q.begin()->first != stdQ.begin()->first ||
            Q.begin()->second != stdQ.begin()->second)
            return 0;
        Q.erase(Q.begin());
    ...
    for (int i = 1; i <= 6000; i++)
        swap(num[rand() % 3000 + 1], num[rand() % 3000 + 1]);
    for (int i = 1; i <= 3000; i++) {
        if (left[num[i]]) {
            if (stdQ.count(num[i])) {
                it = Q.find(num[i]);
...
bool check9() {  // just have fun!
    ...
    if (itQ == itO) return 0;
    // std::cerr << "ok here\n";
    if (citQ == citO) return 0;
    if (!(itQ != itO)) return 0;
    if (!(citQ != citO)) return 0;
    if (itQ == citO) return 0;
    if (itO == citQ) return 0;
    if (!(itQ != citO)) return 0;
    if (!(itO != citQ)) return 0;
    if (!(citQ == itQ)) return 0;
    if (citQ != itQ) return 0;
    return 1;
}
