...
class TestCore {
   ...
   public:
    TestCore(const char* title, const int& id, const int& total)
        : title(title),
          id(id),
          total(total),
          dfn(clock()),
...
void tester1() {
    ...
    try {
        ...
        for (int i = 0; i < (int)ret.size(); i++) {
            ...
            for (int c = 0; c < 10; c++) {
                ...
                if (stdmap[ret[p]] != srcmap[ret[p]]) {
                    // std::cerr << ret[p] << " ";
                    // std::cerr << stdmap[ret[p]] << " " << srcmap[ret[p]] <<
                    // std::endl;
                    console.fail();
                    return;
                }
        ...
        for (auto itA = stdmap.begin(); itA != stdmap.end(); ++itA, ++itB) {
            if ((itA->first).val != (itA->first).val ||
                (itB->first).val != (itB->first).val) {
                console.fail();
                return;
            }
            console.showProgress();
...
void tester5() {
    TestCore console("Error throwing B - Invalid Const_Iterator testing...", 5,
                     0);
    console.init();
    auto ret = generator(MAXN);
    try {
...
void tester6() {
    TestCore console("Error throwing C - Invalid Index testing...", 6,
                     2 * MAXN);
    console.init();
    auto ret = generator(MAXN);
    try {
...
void tester7() {
    ...
    TestCore console("Copy constructure testing...", 7,
                     MAXN + MAXC + 2 * (MAXN - MAXC));
    console.init();
    auto ret = generator(MAXN);
    try {
        std::map<IntA, IntB, Compare> stdmap;
...
void tester8() {
    const int MAXC = MAXN / 2;
    TestCore console("Operator = testing...", 8,
                     MAXN + MAXC + 2 * (MAXN - MAXC));
    console.init();
    auto ret = generator(MAXN);
    try {
...
void tester11() {
    ...
    try {
        ...
        for (int i = 0, cnt = 0; i < (int)ret.size(); i++, cnt++) {
            int tmp = rand();
            auto retA = stdmap.insert(std::map<IntA, IntB, Compare>::value_type(
                IntA(ret[i]), IntB(tmp)));
            auto retB =
                srcmap.insert(sjtu::map<IntA, IntB, Compare>::value_type(
                    IntA(ret[i]), IntB(tmp)));
            console.showProgress();
            if (!retA.second) {
                cnt--;
