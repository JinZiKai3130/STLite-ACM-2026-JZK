...
template <typename Ans, typename Test>
void randnumFill(Ans& ans, Test& test, int n = 100000) {
    for (int i = 0; i < n; i++) {
        int x = randnum();
        if (randnum() % 2) {
            ans.push_back(x);
...
bool iteratorTest() {
    ...
    for (int i = 0; i < MAX_N; i++) {
        ...
        for (int j = 0; j < 2; j++)
            if (*ansIter[j] != *myIter[j]) return false;

        if (ansIter[1] - ansIter[0] != myIter[1] - myIter[0]) return false;
    }
...
bool nomercyTest() {
    ...
    ansCounter = myCounter = 0;
    {
        ...
        if (!dfs(19960904, ans, deq)) return false;
        ...
        if (!dfs2(19960904, ans2, deq2)) return false;
    }
...
int main() {
    bool (*testFunc[])() = {
        ...
        eraseTest, copyAndClearTest, memoryTest, nomercyTest,
    };

    const char* testMessage[] = {
        "Testing push...",   "Testing pop...",
        "Testing insert...", "Testing iterator...",
        "Testing erase...",  "Testing copy and clear...",
        "Testing memory...", "Final test without mercy...",
    };
