...
void TestMatrix() {
    ...
    for (int i = 0; i < N; ++i) {
        vM.push_back(Diamond::Matrix<double>(
            randNum(i + 1, 10 + 7), randNum(i + 2, 10 + 7),
...
void TestCopyConstructorAndOperatorEqu() {
    ...
    dualInt_oper = dInt;
    for (long long i = 0; i < N; ++i) {
        if (dualInt[i] != dInt[i] || dualInt_oper[i] != dInt[i]) error();
    }
    dualInt_oper = dualInt_oper;
    delete pInt;
    for (long long i = 0; i < N; ++i) {
        if (dualInt_oper[i] != dualInt[i]) error();
    }
    std::cout << "Correct." << std::endl;
}

void TestIteratorSequenceAccess() {
    std::cout << "Test 5 : Test for accessing the container in the order of "
                 "the sequence, using iterator...";
    ...
    if (it != dInt.end()) error();
    std::cout << "Correct." << std::endl;
}

void TestIteratorRandomAccess() {
    std::cout << "Test 6 : Test for accessing the container randomly, using "
                 "iterator...";
    sjtu::deque<long long> dInt;
...
void TestPopAndPush() {
    ...
    for (size_t i = 0; i < 1007LL; ++i) {
        if (!(dInt[i] == vInt[i])) error();
        if (!(drInt[1006LL - i] == rInt[i])) error();
    }
...
int main() {
    ...
    TestInsertAndErase();
    TestPopAndPush();
    std::cout << "Congratulations. Your submission has passed all correctness "
                 "tests. Use valgrind to ensure that there ain't any memory "
                 "leaks in your deque. Good job! :)"
              << std::endl;
    return 0;
}
